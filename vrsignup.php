<?php
   ...
   if($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      if ($flag == 1) {
         ...
         $sql = "SELECT * from volunteers where email = '$newemail'";
         $result = mysqli_query($db,$sql);
         $count = mysqli_num_rows($result);
         if ($count == 1) {
            $error = "Email already exists";
         }else {
            $sql1 = "INSERT INTO volunteers (name,mobile,email,password,city,service) VALUES ('$newname',$newmobile,'$newemail','$newpassword','$newcity','$newservice')";
